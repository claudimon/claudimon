...
/* ============================================
   PATH TRACKING
   We store the path segments so the prompt
   can show claudimon/folder/subfolder>
   ============================================ */
#define MAX_DEPTH 16
#define MAX_SEG   32
...
void path_push(const char* name) {
...
void path_reset(void) { path_depth = 0; }

/* ============================================
   MULTIBOOT MEMORY MAP
   GRUB passes memory info in registers — we
   read it via inline asm to report RAM size.
   We store it in kernel.c and expose it here.
   ============================================ */
...
static void cmd_info(void) {
    ...
    terminal_write("\n=== System Information ===\n");
    terminal_set_color(theme_text_color, VGA_COLOR_BLACK);
    terminal_write("  Kernel     : Claudimon v0.3.0\n");
...
static void cmd_play(const char* args) {
    if (!args || !*args) { terminal_write("Usage: play <frequency>\n"); return; }
...
static void cmd_cd(const char* args) {
    ...
    if (k_strcmp(args, "..") == 0) {
        if (fs_cd("..") == 0) path_pop();
    } else if (k_strcmp(args, "/") == 0) {
        if (fs_cd("/") == 0) path_reset();
    } else {
        if (fs_cd(args) == 0) path_push(args);
    }
}
...
/* ============================================
   PROMPT — shows claudimon/folder/subfolder>
   ============================================ */
static void print_prompt(void) {
    ...
    for (int i = 0; i < path_depth; i++) {
        terminal_write("/");
        terminal_write(path_segments[i]);
    }
    terminal_set_color(VGA_COLOR_LIGHT_GREY, VGA_COLOR_BLACK);
    terminal_write("> ");
    terminal_set_color(theme_text_color, VGA_COLOR_BLACK);
